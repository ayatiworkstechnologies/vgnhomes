...
export default function EnquiryForm() {
  ...
  const onSubmit = async (data) => {
    try {
      const payload = {
        ...data,
        mobile: data.mobile.replace(/^91/, ""), // remove country code
      };
            ...
            <div className="row  align-items-start mb-3 gx-4">
              <div className="col-auto d-flex align-items-start">
                ...
                <div>
                  <label
                    htmlFor="consent"
                    className="form-check-label small m-0"
                  >
                    I authorize VGN and its representatives to contact me with
                    updates and notifications via email, SMS, RCS, WhatsApp &
                    call. This will override the registry on DND/NDNC.
                    <span className="">*</span>
                  </label>
                  {errors.consent && (
                    <div className=" small mt-1">{errors.consent.message}</div>
                  )}
                </div>
              </div>
            </div>
